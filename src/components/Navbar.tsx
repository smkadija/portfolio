...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div
          className={`pointer-events-auto relative w-full max-w-6xl transition-all duration-300 rounded-[32px] sm:rounded-full flex items-center px-4 sm:px-7 ${
            isScrolled
              ? 'glass-navbar-scrolled h-[60px] sm:h-[64px]'
              : 'glass-navbar h-[64px] sm:h-[68px]'
          }`}
          data-cursor="nav"
        >
          {/* LEFT — BRAND */}
            ...
            <div className="hidden md:flex items-center gap-0.5 lg:gap-1">
              {navLinks.map((link) => (
                <a
                  ...
                  className="px-3 py-1.5 text-xs sm:text-sm font-medium tracking-tight text-[#8E8287] hover:text-white hover:drop-shadow-[0_0_8px_rgba(214,106,137,0.7)] transition-all rounded-full hover:bg-white/[0.04]"
                >
                  {link.label}
                </a>
              ))}
